...
def user(ctx, username, password, endpoint):
    ...
    if not endpoint:
        endpoint = click.prompt("Server endpoint URL")
...
def list_nodes(ctx):
    ...
    try:
        ...
        if isinstance(response, dict) and response.get("status") == "failure":
            click.echo(f"Error listing nodes: {response.get('message', 'Unknown error')}", err=True)
            return
...
def update(ctx, endpoint):
    """Update server HTTP base URL in configuration"""
    try:
        config_manager = ConfigManager(ctx.obj.get('config_id'))
        
        if endpoint:
            config_manager.update_base_url(endpoint)
            # Update the API client's base URL
            ctx.obj['api_client'].base_url = config_manager.get_base_url()
            click.echo("Server configuration updated successfully")
            click.echo(f"HTTP Base URL: {config_manager.get_base_url()}")
        else:
            click.echo("No endpoint provided. Current HTTP Base URL:", config_manager.get_base_url())
...
@click.pass_context
def reset(ctx):
    """Reset server HTTP base URL to Espressif public URL"""
    try:
        config_manager = ConfigManager(ctx.obj.get('config_id'))
        ...
        click.echo("Server endpoint reset to Espressif public URL")
        click.echo(f"HTTP Base URL: {config_manager.get_base_url()}")
...
def cleanup(ctx, days):
    ...
    try:
        config_manager = ConfigManager()
        config_manager.cleanup_old_configs(max_age_days=days)
        click.echo(f"Cleaned up configuration files older than {days} days")
    except Exception as e:
        click.echo(f"Failed to cleanup old configurations: {str(e)}", err=True)
