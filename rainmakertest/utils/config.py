"""Configuration utilities for the Rainmaker CLI."""
...
def get_config_path(config_id: Optional[str] = None) -> Path:
...
def get_token_path(config_id: Optional[str] = None) -> Path:
    """Get the path to a token file."""
    config_dir = get_config_dir()
    if not config_id:
        return config_dir / "token.json"
    return config_dir / f"{config_id}_token.json"